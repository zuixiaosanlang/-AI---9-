...
max_accuracy = 0
schedule = lr.MultiStepDecay(learning_rate=1e-3, milestones=Config["solver"]["milestones"],
...
with fluid.dygraph.guard():
    ...
    try:
        for epoch in range(0, Config["solver"]["num_epoch"]):
            ...
            for i, (img, labels) in enumerate(train_loader):
                ...
                if i % Config["solver"]["loss_print_freq"] == 0:
                    print("epoch {}/{} iter {} loss: {} train_acc {}".format(epoch, num_epoch, i, avg_loss.numpy(),
                                                                             acc.numpy()))
        ...
        save_p = os.path.join(save_dir, 'angleClass_' + str(Config["solver"]["num_epoch"]))
        model_state = net.state_dict()
        paddle.save(model_state, save_p)
